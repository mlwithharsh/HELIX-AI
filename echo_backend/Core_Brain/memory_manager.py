...
class MemoryManager:
    ...
    def add_memory(self, user, echo, session_id=None, analysis=None, reward=None, reflection=None):
        try:
            if not session_id:
            ...
            importance = self.score_importance(user, echo, analysis or {}, reward)
            if importance >= 0.6:
    ...
    def score_importance(self, user_text, echo_text, analysis, reward):
        score = 0.2
    ...
    def get_relevant_memories(self, query=None, limit=3):
        ...
        for memory in self.long_term_memories:
            content = f"{memory.get('user', '')} {memory.get('echo', '')}".lower()
